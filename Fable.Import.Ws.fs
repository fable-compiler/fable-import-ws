...
//        abstract perMessageDeflate: U2<bool, IPerMessageDeflateOptions> option with get, set
    ...
    and [<AllowNullLiteral>] [<Import("Server","WebSocket")>] Server =
        ...
        [<Emit("$0.on('error',$1...)")>] abstract on_error: Error -> unit //(cb: Func<Error, unit>): obj = failwith "JS only"
        [<Emit("$0.on('headers',$1...)")>] abstract on_headers: ResizeArray<string> -> unit //(cb: Func<ResizeArray<string>, unit>): obj = failwith "JS only"
        [<Emit("$0.on('connection',$1...)")>] abstract on_connection: WebSocket -> unit //(cb: Func<WebSocket, unit>): obj = failwith "JS only"
        abstract on: ``event``: string * listener: (obj -> unit) -> unit
...
//        abstract addListener: string -> obj -> unit
    ...
    type [<Import("*","WebSocket")>] Globals =
        static member createServer(?options: IServerOptions, ?connectionListener: Func<WebSocket, unit>): Server = failwith "JS only"
        static member connect(address: string, ?openListener: Function): unit = failwith "JS only"
        static member createConnection(address: string, ?openListener: Function): unit = failwith "JS only"
